...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        )  # 对应((1, 1), (1, top), (right, 1), (right, top))
        ...
        for action in [
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextState in self.corners:
                    finished = list(copy.deepcopy(state[1]))
                    finished[self.corners.index(nextState)] = True
                    finished = tuple(finished)
                else:
                    finished = copy.deepcopy(state[1])
                successors.append(((nextState, finished), action, 1))
        self._expanded += 1  # DO NOT CHANGE
        return successors
